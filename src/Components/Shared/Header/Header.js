...
const Header = () => {
    ...
    console.log(window.addEventListener("focus", console.log('focus')));
    ...
    return (
        <div className={`fixed w-full z-50 ${params !== '/' && 'bg-gray-900 relative'} transition-all duration-500 ${scrollPosition > 150 && "  bg-gray-900 shadow-lg "}`}>
            <nav
                className={`relative w-full flex flex-wrap items-center lg:justify-between py-3  text-gray-200 navbar navbar-expand-lg navbar-light ${(params === "/inventory" || params === "/blogs") ? 'hidden' : 'none'} `}
            >
